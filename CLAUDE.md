...
- Debug panel (F8 toggle, "NPC Sandbox"): Spawn section (4 behavior buttons + Clear All), visual toggles (disc, stateLabel, chasePath, waypoints, waypointNumbers, nameLabel, wanderBeam) with Toggle All master switch, live NPC status readout, teleport (NPC current pos, summon NPC to player), per-NPC behavior swap buttons, per-NPC despawn buttons. Server-driven dynamic NPC list (rebuilds on spawn/despawn). Client polls server every 0.5s + push events for instant state changes. Visual toggle states tracked in `visualToggleStates` map and synced to new NPCs on rebuild. Spawn section built on first poll (deferred, needs `availableBehaviors` from server `_meta`).
