...
NPC pathfinding sandbox — on-demand spawning of chase, patrol, wander, and guard NPCs via debug panel, using PathfindingService.
...
- `NPCManager` — Registry-based NPC orchestrator. Public API: `spawnNPC(config)` (creates rig + registers), `registerNPC(model, config)` (adopts pre-placed model), `despawnNPC(npcId)` (individual teardown), `setBehavior(npcId, behaviorType, behaviorConfig?)` (runtime behavior swap — tears down SM, scrubs ctx, cleans visuals, re-wires), `cleanup()` (tear down all NPCs). Also exposes `getAllNPCStatus()`, `getAvailableBehaviors()`, `setDebugVisualEnabled()`, `setDebugStateCallback()`, `getNPCPosition()`, `teleportNPC()` for debug panel. Internal BehaviorRegistry maps behavior type strings to state modules. State definitions live in `behaviors/` modules.
- `DebugVisuals` — Debug visualization: combined NPC label (`NPCLabelGui` billboard with name + state TextLabels), detection discs, visual toggling (`setVisualEnabled` — uses `TextLabel.Visible` for label toggles since both share one billboard), per-frame debug updates (`updateTick`), cleanup. Extracted from NPCManager to separate presentation from NPC lifecycle.
...
- `NPCAnimator` — Signal-driven animation controller. Loads idle/walk/run/jump/fall tracks from `GameConfig.NPC.ANIMATIONS`, driven by `Humanoid.Running` + `Humanoid.StateChanged` signals. `setup(model)` returns a cleanup closure. No coupling to state machines or pathfinder.
- `DebugRemotes` — String constants for debug remote names (shared, single source of truth). Includes GET_NPC_STATUS, SPAWN_NPC, DESPAWN_NPC, CLEAR_ALL_NPCS, TOGGLE_VISUAL, TELEPORT_TO_NPC_CURRENT, TELEPORT_NPC_TO_PLAYER, SET_BEHAVIOR, NPC_STATE_CHANGED.
- `DebugService` — Server-side handler bridging debug panel requests into NPCManager. Handles NPC spawning (auto-naming `Chase NPC 1`, `Patrol NPC 1`..., position-relative config generation for waypoints/center), despawning, clear-all, visual toggling, teleport, behavior swap. Augments status response with `_meta.availableBehaviors`. SET_BEHAVIOR and SPAWN_NPC use `generateSpawnConfig()` to create position-relative waypoints/center (patrol square, wander center, guard pentagon around spawn/NPC position).
- `DebugPanel` — Client-side debug panel logic ("NPC Sandbox"). Opens by default (F8 toggles). Server-driven dynamic NPC list. Polls status every 0.5s, diffs NPC list, rebuilds status/teleport/summon/behavior/despawn sections on add/remove. Spawn section (deferred build on first poll) with 4 behavior type buttons + Clear All. Summon section (separate from Teleport). Despawn section with per-NPC red buttons. Visual toggle states persist across rebuilds and sync to newly spawned NPCs.
- `DebugPanelUI` — UI factory functions for debug panel elements (raw Roblox instances, no framework). Includes `createSpawnSection` (behavior buttons + clear all), `createDespawnGrid` (per-NPC red buttons), `createBehaviorRow` (per-NPC behavior swap with active highlight).

### NPC Pathfinding System
- Registry-based NPC system: `spawnNPC(config)` creates R15 rigs, `registerNPC(model, config)` adopts pre-placed models, `despawnNPC(npcId)` tears down individual NPCs, `setBehavior(npcId, behaviorType, behaviorConfig?)` swaps behavior at runtime (model + pathfinder persist), `cleanup()` tears down all NPCs. Scene starts empty — NPCs spawned on-demand via debug panel.
...
- R15 rigs created at runtime via `Players:CreateHumanoidModelFromDescription()`, `DisplayDistanceType = None` (native name label suppressed in favor of custom NPCLabelGui)
...
- Debug panel (opens by default, F8 toggle, "NPC Sandbox"): Spawn section (4 behavior buttons + Clear All), visual toggles (disc, stateLabel, chasePath, waypoints, waypointNumbers, nameLabel, wanderBeam) with Toggle All master switch, live NPC status readout, teleport (go-to NPC), summon (bring NPC to player — separate section), per-NPC behavior swap buttons, per-NPC despawn buttons. Server-driven dynamic NPC list (rebuilds on spawn/despawn). Client polls server every 0.5s + push events for instant state changes. Visual toggle states tracked in `visualToggleStates` map and synced to new NPCs on rebuild. Spawn section built on first poll (deferred, needs `availableBehaviors` from server `_meta`). NPC naming format: "Chase NPC 1", "Patrol NPC 2", etc.
...
- NPC animations (idle, walk, run, jump, fall) driven by `Humanoid.Running` and `Humanoid.StateChanged` signals via `NPCAnimator` module. Completely decoupled from behavior state machines — signals fire automatically from `MoveTo` calls. Walk vs run threshold at `RUN_SPEED_THRESHOLD` (default 8): chase NPCs (WalkSpeed 4) walk-animate, guard NPCs (WalkSpeed 10) run-animate. Animation IDs and tuning in `GameConfig.NPC.ANIMATIONS`.
